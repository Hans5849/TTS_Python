...
The package layer now adds source-version identity, repeated size/mtime stability
observations, restart recovery, durable bounded retry state, optional provider lifecycle
management, idle unloading, and physical NVIDIA GPU resolution. These features extend
the modular package rather than replacing the preserved converter. The converter's
audio validation, cache, assembly, metadata, and reporting remain scheduled for careful
extraction behind provider interfaces before compatibility removal is considered.
